 ...
 *
 * Two of them are the deliberate acts: parking billing with a reason, and
 * signing the job off. The other two are the short path — marking the job
 * billed, or billed and paid, without typing an invoice at all. That is a real
 * way of working, not a shortcut around the ledger: work invoiced and collected
 * outside this app still has to be able to leave the desk, and a queue that
 * demands an invoice number, a PO and a send date first is a queue people stop
 * updating. What the mark writes is an ordinary invoice row, so the stage and
 * every total follow from it exactly as they would have by hand — and the
 * ledger below is where it gets a number later, or gets undone.
 */
